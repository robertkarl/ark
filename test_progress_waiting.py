...
class _RecordingTmux(ark.Tmux):
    ...
    def send_command(self, cmd, pane=ark.AGENT_PANE):
    ...
    def respawn_agent_pane(self):
        # Pane-level reset: the session (and the orchestrator in pane 1) stays
        # alive; only the agent pane is reset in place (EC-7).
...
def test_idle_timeout_respawns_stale_pane_so_retry_starts_clean(tmp_path):
    """FR-9 / EC-7 fix: an IDLE_TIMEOUT leaves the tmux session alive. run_in_tmux
    must reset the stale AGENT PANE in place (respawn pane 0) so a subsequent
    retry sends into a fresh prompt instead of the still-live hung pane — while
    leaving the session (and the orchestrator in pane 1) alive."""
    ...
    assert outcome == StepOutcome.IDLE_TIMEOUT
    # The first attempt sent the command, then the stale agent pane was respawned
    # in place. No second send happened on this non-finishing path, and the
    # session was never killed (the orchestrator lives in pane 1 — EC-7).
    assert tmux.events == ["send", "respawn"]
    # Session stays alive: the next attempt reuses it, sending into pane 0.
    assert tmux.is_alive()


def test_idle_timeout_retry_reuses_respawned_pane_not_double_send(
        monkeypatch, tmp_path):
    """The two-call sequence: an idle timeout followed by a finishing retry must
    send into the respawned agent pane of the still-live session rather than
    stacking a second send onto the original hung pane. The session is NEVER
    recreated — recreating it would have killed the orchestrator in pane 1."""
    # The FINISHED branch exits the agent REPL and sleeps; stub both so the test
    ...
    # The retry reused the live session, sending straight into the respawned
    # pane 0 — no "create" (which would have torn down the orchestrator pane).
    assert tmux.events[2:] == ["send"]
    assert "create" not in tmux.events
